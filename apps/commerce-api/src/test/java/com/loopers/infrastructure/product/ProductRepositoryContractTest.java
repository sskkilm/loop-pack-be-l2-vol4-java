...
abstract class ProductRepositoryContractTest {
    ...
    abstract ProductRepository repository();
    ...
    void decreaseLikeCount_doesNotDecreaseBelowZero_whenLikeCountIsZero() {
        // given
        ProductModel product = repository().save(new ProductModel(1L, "상품A", BigDecimal.valueOf(10000)));

        // when
        repository().decreaseLikeCount(product.getId());

        // then
        ProductModel found = repository().find(product.getId()).orElseThrow();
        assertAll(
